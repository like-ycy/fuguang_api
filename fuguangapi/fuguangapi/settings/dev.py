...
# 新增apps作为导包路径，导包路径默认保存sys.path属性中，
# 所有的python的import或者from导包语句默认都是从sys.path中记录的路径下查找模块
...
SECRET_KEY = 'django-insecure-3ax8vq5o(=*m4v@byc)oi4a9d$t04%s5m#$^29mb0^6$ab1ux9'
...
CACHES = {
    ...
    "default": {
        ...
        # 项目上线时，需要调整这里的路径
        "LOCATION": "redis://dba:123.com@127.0.0.1:6379/0",
        "OPTIONS": {
    ...
    "session": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://dba:123.com@127.0.0.1:6379/1",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    ...
    "sms_code": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://dba:123.com@127.0.0.1:6379/2",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    ...
    "hot_word": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://dba:123.com@127.0.0.1:6379/3",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    ...
    "cart": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://dba:123.com@127.0.0.1:6379/4",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    ...
    "coupon": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://dba:123.com@127.0.0.1:6379/5",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
...
# True为使用系统时区，False使用上面TIME_ZONE设置的时区
USE_TZ = False
...
JWT_AUTH = {
    # 设置JWT的有效期
    # 'JWT_EXPIRATION_DELTA': datetime.timedelta(weeks=1),  # 一周有效期
    'JWT_EXPIRATION_DELTA': datetime.timedelta(minutes=30),  # 30分钟有效期
    # 自定义载荷
...
TENCENTCLOUD = {
    # 腾讯云访问秘钥ID
    "SecretId": "AKIDpYAweATaT9aqtjLiqjS4b5QgoiQwGXeZ",
    # 腾讯云访问秘钥key
    "SecretKey": "qfMvPl160pZBcrvzzqW9jANNAl9NXffW",
    # 验证码API配置
    "Captcha": {
...
# 容联云短信配置
RONGLIANYUN = {
    "accId": '8a216da87ce04099017d227eb9f00d2d',
    "accToken": '950d0942124f4262ab5b0e86fc5d4570',
    "appId": '8a216da87ce04099017d227ebafa0d34',
    "reg_tid": 1,  # 注册短信验证码的模板ID
    "sms_expire": 300,  # 短信有效期，单位：秒(s)
    "sms_interval": 60,  # 短信发送的冷却时间，单位：秒(s)
}

# 子应用users的celery配置项[注意：django的配置项必须大写，所以这里的所有配置项必须全部大写]

# 任务队列
CELERY_BROKER_URL = 'redis://dba:123.com@127.0.0.1:6379/14'
# 结果队列
CELERY_RESULT_BACKEND = 'redis://dba:123.com@127.0.0.1:6379/15'
# 时区，与django的时区同步
...
# 首页图标地址
SIMPLEUI_INDEX = 'http://www.fuguang.com:3000/'

# 阿里云OSS配置
OSS_ACCESS_KEY_ID = "LTAI5tRgBHTg3rGmnAcFwmwk"
OSS_ACCESS_KEY_SECRET = "XTFtpa2nPPtModcX4v7kR7Ewj7U6ac"
OSS_ENDPOINT = "oss-cn-beijing.aliyuncs.com"  # 访问域名, 根据服务器上的实际配置修改
OSS_BUCKET_NAME = "fuguang-online"  # oss 创建的 BUCKET 名称
...
ALIPAY = {
    ...
    "return_url": "http://www.fuguang.com:3000/alipay",  # 同步回调结果通知地址
    "notify_url": "http://api.fuguang.com:8000/payments/alipay/notify",  # 异步回调结果通知地址
}

# 保利威视频加密服务, 有效期只2021-12-10
POLYV = {
    "userId": "705f4d7a89",
